...
update_system_message = |client, messages|
    when client.api is
        Anthropic ->
            system = option_to_str(client.system)
            new_system =
                List.keep_if(messages, |message| (message.role == "system") and !Str.contains(system, message.content))
                |> List.map .content
                |> List.prepend(system)
                |> Str.join_with("\\n")
...
to_compatible_messages : List Message, ApiTarget -> List Message
...
decode_response = |body_bytes|
    ...
    List.walk_until(
        [
            decode_default_response,
            decode_anthropic_response,
            decode_ollama_response,
        ],
        Err(TooShort),
        |_state, decoder|
            when decoder(cleaned_body) is
                Ok(response) -> Break(Ok(response))
                Err(_) -> Continue(Err(TooShort)),
    )
...
decode_ollama_response : List U8 -> Result ChatResponseBody [TooShort]
decode_ollama_response = |body_bytes|
    decoder = Json.utf8_with({ field_name_mapping: SnakeCase })
    decoded : Decode.DecodeResult DecodeOllamaResponseBody
    decoded = Decode.from_bytes_partial(body_bytes, decoder)
    decoded.result |> Result.map_ok(from_ollama_response)
...
from_ollama_response : DecodeOllamaResponseBody -> ChatResponseBody
from_ollama_response = |internal_response| {
    id: "none",
    model: internal_response.model,
    object: "none",
    created: internal_response.created_at
    |> DateTime.from_iso_str
    |> Result.with_default(DateTime.unix_epoch)
    |> DateTime.to_nanos_since_epoch
    |> Num.to_u64,
    choices: [
        {
            index: 0,
            message: convert_decode_message(internal_response.message),
            finish_reason: internal_response.done_reason,
        },
    ],
    usage: {
        prompt_tokens: 0,
...
convert_decode_message = |decode_message| {
    ...
    reasoning_content: option_to_str(decode_message.reasoning_content),
...
add_system_message = |client, text, { cached ?? Bool.false }|
    when client.api is
        ...
        _ ->
            updated = List.append(client.messages, { role: "system", content: text, reasoning_content: "", tool_calls: [], tool_call_id: "", name: "", cached })
...
add_user_message = |client, text, { cached ?? Bool.false }|
    updated = List.append(client.messages, { role: "user", content: text, reasoning_content: "", tool_calls: [], tool_call_id: "", name: "", cached })
    { client & messages: updated }
...
add_assistant_message = |client, text, { cached ?? Bool.false }|
    updated = List.append(client.messages, { role: "assistant", content: text, reasoning_content: "", tool_calls: [], tool_call_id: "", name: "", cached })
    { client & messages: updated }
