...
inject_tools = \request_body, tools ->
    ...
    [before, (", \"tools\": " |> Str.to_uf8), tools_json, others] |> List.join
...
encode_tools = \tools ->
    ...
    |> Str.join_with (", ")
    |> \tools_content -> "[$(tools_content)]"
    |> Str.to_utf8

## Convert a tool object to a JSON string.
tool_to_json : Tool -> Str
tool_to_json = \tool ->
    required = Encode.to_bytes(tool.function.required, Json.utf8With({ fieldNameMapping: SnakeCase })) |> Str.from_utf8 |> Result.with_default("[]")
    """
